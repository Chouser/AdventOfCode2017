...
(defn step [{:keys [prog played ip inbox] :as s}]
  (let [[op x y] (get prog ip)
        ;;_ (prn op x y (-> s (dissoc :prog) (update :inbox seq)))
        v (fn [string]
            (if (re-matches #"[a-z]" string)
              (get s string 0)
              (Long/parseLong string)))
...
(defn run-pair [prog]
  (loop [ss [(assoc init "p" 0 :prog prog)
    ...
    #_(clojure.pprint/pprint {:ss (map #(dissoc % :prog) ss)})
